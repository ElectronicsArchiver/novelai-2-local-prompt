...
function convert ( input ){
    ...
    for ( const match of text.matchAll(re_attention) ){
        ...
        if(word === '}' && curly_brackets.length > 0){
            multiply_range(curly_brackets.pop(), curly_bracket_multiplier);
            continue
        }
        
        if(word === ']' && square_brackets.length > 0){
...
function onClickConvert (){
    ...
    let prompt = gradioApp()
        .querySelector('#txt2img_prompt > label > textarea');
    ...
    let negprompt = gradioApp()
        .querySelector('#txt2img_neg_prompt > label > textarea');
...
onUiUpdate(() => {
    ...
    let parentArea = gradioApp()
        .querySelector('#toprow');
    
    let generateBtn = gradioApp()
        .querySelector('#txt2img_generate');
    
    let beforeElement = gradioApp()
        .querySelector('#roll_col');
    ...
    if(gradioApp().querySelector('#nai2local') != null)
        return;
    ...
    nai2LocalArea.style = 'min-width: min(110px, 100%); max-width: 120px; flex-grow: 1; padding: 0em; padding-top: 0.4em;';
    ...
    convertBtn.style = 'padding-left: 0.1em; padding-right: 0em; margin: 0.1em 0;max-height: 2em; max-width: 6em';
    ...
    undoBtn.style = 'padding-left: 0.1em; padding-right: 0em; margin: 0.1em 0;max-height: 2em; max-width: 6em';
